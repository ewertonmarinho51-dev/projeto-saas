# Autorização aberta no banco — diagnóstico e plano (P0 preventiva)
...
(`db._cliente()`). E **26 das 28 tabelas** do schema `public` têm
políticas RLS concedendo a `anon` **SELECT, INSERT, UPDATE e DELETE
irrestritos** (`using (true)` / `with check (true)`).

> **Correção de uma afirmação anterior deste relatório.** A primeira
> versão dizia que a chave publicável "por definição vai para o
> navegador de qualquer visitante". **Isso está errado para esta
> arquitetura.** O GovDocs é Streamlit e roda inteiro no servidor: o
> navegador recebe a interface já renderizada, por websocket, e
> `st.secrets` **nunca é enviado ao cliente**.
>
> O que permanece verdadeiro é o que sustenta o achado: a chave
> publicável **deve ser tratada como pública** — é a premissa do modelo
> do Supabase, que coloca a autorização no RLS — e ela circula por
> painel, backups de configuração, logs de plataforma e pela API de
> gestão do projeto. **A vulnerabilidade material é a combinação de
> grants e políticas permissivas**, não a forma como a chave chega ao
> processo.
>
> **Nada aqui está provado quanto a exposição ou comprometimento.** Não
> há evidência de que a chave tenha vazado nem de que as tabelas tenham
> sido acessadas por terceiros — e também não há trilha que permita
> afirmar o contrário. A classificação correta é **P0 preventiva**, e
> está detalhada em `docs/seguranca-achado-p0.md`, §1.
...
`has_table_privilege` na produção em
**14/08/2026** — somente leitura.

---

## 2. `usuarios`: tomada de conta é possível — sem prova de que ocorreu
...
**A tomada de conta é POSSÍVEL; não há prova de que tenha ocorrido.**
Não existe evidência de exploração — e também não existe trilha que
permita afirmar o contrário: as políticas não registram autoria e o app
não mantém log de acesso. Ausência de evidência não é evidência de
ausência, nem de ocorrência.

Por isso a revisão de contas entra como medida **precaucional**, não
como remediação de um comprometimento constatado: na etapa 1, procurar
conta desconhecida, `papel` inesperado ou `criado_em` fora do padrão, e
forçar a redefinição de todas as senhas. Se algo disso aparecer, aí sim
a classificação muda para incidente.
...
### Etapa 1 — Rotação precaucional das chaves de IA
...
- *Pré-requisito de tudo: fechar o acesso agora não alcança quem já
  tenha obtido a chave antes — e não há como saber se alguém obteve.*
